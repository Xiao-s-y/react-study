...
# React 语法演进：从类组件到 Hooks（2020 vs 现在）

下文旨在总结 **React 经典写法（以 2020 年前后类组件为主流）** 与 **当前（2026）推荐的现代函数式 Hooks 写法** 之间的核心差异，并提供迁移指南。

---

## 📌 概览

| 特性维度 | 2020 年前后（Class Component） | 现在（Function Component + Hooks） |
| :--- | :--- | :--- |
| 组件定义 | `class X extends React.Component` | `const X = () => {}` 或 `function X() {}` |
| 局部状态 | `this.state = {}` + `this.setState()` | `useState` |
| 副作用/生命周期 | `componentDidMount`, `componentDidUpdate`, `componentWillUnmount` | `useEffect`（统一管理） |
| 上下文 | `this.context` 或 `<Context.Consumer>` | `useContext` |
| Redux 集成 | `connect` 高阶组件（HOC） | `useSelector` + `useDispatch` |
| DOM 引用 | 字符串 ref / 回调 ref / `React.createRef` | `useRef` |
| 方法绑定 | 需要 `bind(this)` 或使用箭头函数类属性 | 箭头函数直接定义（闭包捕获） |
| 性能优化 | `PureComponent` / `shouldComponentUpdate` | `React.memo` + `useMemo` / `useCallback` |
| 逻辑复用 | 高阶组件（HOC） / Render Props | 自定义 Hooks |
| 类型支持（TypeScript） | `React.Component<P, S>` | `React.FC<P>` 或直接函数签名 |
...
class Input extends Component {
  constructor() {
    super();
    this.inputRef = React.createRef();   // 或使用回调 ref
  }
  focus = () => {
...
**类组件（2020）**
- 继承 `React.PureComponent`（浅比较 props 和 state）
- 或手动 `shouldComponentUpdate`

**函数组件（现在）**
- 使用 `React.memo` 包裹组件（浅比较）
- 使用 `useMemo` 缓存计算结果
- 使用 `useCallback` 缓存函数引用

```jsx
const MemoizedChild = React.memo(({ data, onAction }) => {
  // 只有当 data 或 onAction 变化时才会重渲染
});
...
**类组件（2020）**  
- 高阶组件（HOC）：`withAuth(Component)`  
- Render Props：`<DataProvider render={data => ...} />`

**函数组件（现在）**  
- **自定义 Hook**：将逻辑提取到 `useXxx` 函数中，多个组件可共享。
...
> 注意：`React.FC` 隐式包含 `children`，在新版本中已不推荐，建议显式声明。
...
| 项目 | 2020 写法 | 现在写法 |
| :--- | :--- | :--- |
| 默认 props | `Component.defaultProps = {}` | 函数参数默认值 `({ name = 'Guest' })` |
| PropTypes | 使用 `prop-types` 库 | 推荐 TypeScript，或保留 PropTypes |
| 事件传参 | `onClick={this.handleClick.bind(this, id)}` | `onClick={() => handleClick(id)}` |
| 状态合并 | `setState(prev => ({ ...prev, key: val }))` | `useState` 不会自动合并，需手动展开或使用 `useReducer` |

---

## 🚀 迁移建议

1. **从类组件迁移到函数组件**：可以逐步进行，优先迁移无状态或简单组件。
2. **Redux**：从 `connect` 迁移到 `useSelector/useDispatch`，但 `connect` 依然可用，新项目推荐 Hooks。
3. **生命周期**：将 `componentDidMount` + `componentDidUpdate` + `componentWillUnmount` 合并为 `useEffect`，注意依赖数组。
4. **性能优化**：用 `React.memo` 替代 `PureComponent`，用 `useCallback/useMemo` 缓存。
5. **逻辑复用**：将 HOC 或 Render Props 改写为自定义 Hooks，更直观。

---

## 📚 参考资料

- [React 官方文档 – 迁移到 Hooks](https://react.dev/reference/react)
- [Redux 官方文档 – Hooks 用法](https://redux.js.org/introduction/why-rtk-is-redux-today)
- [React 版本发布日志](https://react.dev/blog)

---

**总结**：虽然类组件在 React 18+ 中依然被支持，但函数组件 + Hooks 已成为社区标准，新项目应优先采用。本文档覆盖了绝大多数日常开发中的差异点，可作为迁移或学习参考。
